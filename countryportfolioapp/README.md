# countryportfolioapp 

This is a simple app that display a collection of information about countries around the world according to https://restfulcountries.com/


## Project setup
This project displays a list on countries, and on clicking any country, it displayes a few imformation about the country.

- The state management in this project is the bloc state management

- Architecture in use is MVC: Where the view is seperated from the project logic for scalability, sustainability and seamless team collaborations.

- We have various models in use for determining the display structure of data on the UI, you can find all models in the models directory (lib/models/)

- View files can be found in the presentation directory(lib/presentation)

- services and business logic can be found in the services directory (lib/services)

- Theme customization details can be found in the theme directory (lib/theme)

## Getting Started
To run this application on your local machine, kindly run the following command in your terminal:
- git clone https://github.com/Isaac-Franklin/HNG.git
- cd countryportfolioapp
- flutter pub get
- flutter run


## Contribute to this project
- thank you for your interest in contributing to this project, kindly ready this full contribution guidlines here as well as the instruction below before contributing: 

[See contribution guidelines here] (https://github.com/Isaac-Franklin/HNG/blob/main/CONTRIBUTING.md)

- [Kindly go here to fill form and generate an API key] (https://restfulcountries.com/request-access-token)

- [View API documentation] (https://restfulcountries.com/api-documentation/version/1)

- Use the instruction above to run project on your local machine and contribute via a unique branch with proper commit messages detailing your contributions


